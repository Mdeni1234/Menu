  ...
  ngOnInit() {
    this.serviceMenu.getMenu().subscribe(data => {
      this.menu = data.map(x => {
        return {
          id: x.payload.doc.id,
          ...x.payload.doc.data() as Menu
        } 
      })
      console.log(this.menu) 
      this.setPesanan(this.menu);
    });
    this.create();    
    }
  ...
  tambah(index, pesanan:number) {
    ...
    jumlah(z) { 
      ...
      for (let a = 0; a < z; a++) {
        ...
        this.serviceMenu.addPesanan(z,hasil, this.form.value);
